...
export abstract class APIClient {
  ...
  async buildRequest<Req>(inputOptions: FinalRequestOptions<Req>): Promise<{ req: RequestInit; url: string; timeout: number }> {
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new PromptFoundryError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new PromptFoundryError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null)
    throw new PromptFoundryError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new PromptFoundryError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`PromptFoundry:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new PromptFoundryError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
